...
# Start postgresql
su postgres -c '/usr/lib/postgresql/9.6/bin/postgres -D /var/lib/postgresql/9.6/main -c config_file=/etc/postgresql/9.6/main/postgresql.conf >/var/log/postgresql/postgresql-9.6-main.log 2>&1 &'
sleep 30

# Start Blynk
java -jar /blynk/server.jar -dataFolder /data -serverConfig /config/server.properties >/blynk/blynk.log 2>&1 &
